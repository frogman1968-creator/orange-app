...
export function LeagueProvider({ children }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) { setNotConnected(true); setLoading(false); return; }

      const json = await res.json();
      const { teams, connected } = json;
      if (!teams?.length && !connected) { setNotConnected(true); setLoading(false); return; }
