...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      // Report the real failure instead of masking it as a successful,
      // empty response — a 200 here made genuine Yahoo API errors look
      // identical to "off-season, no data yet" in the UI.
      return res.status(502).json({ error: 'Yahoo API error - try again later', reconnect: false });
    }
