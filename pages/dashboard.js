...
function Dashboard() {
  ...
  const { leagues, selected, setSelected, loading: leagueLoading, notConnected, apiError: leagueApiError } = useLeague();
  ...
  return (
    <div style={styles.page}>
      ...
      {view === 'matchup'   && <MatchupView matchup={matchup} hasError={!!leagueApiError || !!dataError} />}
...
function MatchupView({ matchup, hasError }) {
  ...
  if (!matchup) {
    return (
      <div style={{ ...styles.section, paddingTop: 24, textAlign: 'center' }}>
        ...
        {hasError ? (
          <>
            <div style={{ color: '#ef4444', fontWeight: 700, fontSize: 15, marginBottom: 6 }}>Couldn't Load Matchup</div>
            <div style={{ color: '#52525b', fontSize: 13, lineHeight: 1.5 }}>
              There was a problem reaching Yahoo. Pull to refresh or try again shortly.
            </div>
          </>
        ) : (
          <>
